      ...
      data.forEach((item) => {
        ...
        const imageSrc =
          item.images && item.images.length > 0
            ...
            : `logo-removebg-preview.png?text=${
                isLost ? "عنصر+مفقود" : "عنصر+موجود"
              }`;
        ...
        card.innerHTML = `
                    <div class="lost-item-card card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <button class="btn btn-danger delete-btn"  data-id="${item.id}">حذف🗑️</button>
                        </div>
                        <img src="${imageSrc}" class="item-image card-img-top" alt="${
          item.title
        }">
                        <div class="cardd-body p-2">
                            <h5 class="t-title">${item.title}</h5>
                            <p class="t-text"><strong>التفاصيل:</strong> ${
                              item.description
                            }</p>
                            <ul class="list-group list-group-flush mb-3">
                                <li class="list-group-item"><i class="bi bi-calendar"></i> <strong>التاريخ:</strong> ${
                                  item.date.split("T")[0]
                                }</li>
                                <li class="list-group-item"><i class="bi bi-geo-alt"></i> <strong>المكان:</strong> ${
                                  item.location
                                }</li>
                                <li class="list-group-item"><i class="bi bi-person"></i> <strong>التواصل:</strong> ${
                                  item.phoneNumber
                                }</li>
                            </ul>
                        </div>
                        <button class="btn btn-primary btn-sm chat-modal" onclick="startChat(${userId}, ${
          item.userId
        })">
            ...
            if (result.isConfirmed) {
              try {
                const res = await fetch(
                  `https://localhost:7259/api/LostItems/${item.id}?userId=${userId}&isAdmin=true`,
                  {
                    method: "DELETE",
                  }
                );
                ...
                if (res.ok) {
                  ...
                  fetchLostItems(); // ← إعادة تحميل العناصر بعد الحذف
                } else {
                  const errorMessage = await res.text(); // ← عرض رسالة الخطأ من السيرفر
